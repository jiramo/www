...
export default function DownloadPage() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#050505] font-sans selection:bg-orange-500/30 text-neutral-200 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-24 md:py-32">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <p className="text-lg text-neutral-400 leading-relaxed max-w-xl mx-auto">
            Native performance. Local-first architecture. <br className="hidden md:block"/>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-350 mx-auto mb-32">
          ...
          >
            <div className="flex flex-col gap-4 mt-auto">
                <div className="bg-white/5 p-1 rounded-lg flex border border-white/5">
                    <button onClick={() => setMacArch("silicon")} className={`flex-1 text-[10px] font-bold py-1.5 rounded-md transition-all ${macArch === "silicon" ? "bg-white text-black shadow-sm" : "text-neutral-500 hover:text-white"}`}>Apple Silicon</button>
                    <button onClick={() => setMacArch("intel")} className={`flex-1 text-[10px] font-bold py-1.5 rounded-md transition-all ${macArch === "intel" ? "bg-white text-black shadow-sm" : "text-neutral-500 hover:text-white"}`}>Intel</button>
                </div>
                <DownloadButton primary={activeOS === "macos"} label="Download for Mac" />
                <p className="text-center text-[10px] text-neutral-600 font-mono">Requires macOS 11+</p>
            </div>
          </OSCard>
          ...
          >
            <div className="flex flex-col gap-4 mt-auto">
                ...
                <DownloadButton primary={activeOS === "windows"} label="Download for Windows" />
                <div className="text-center">
                    <Link href="#" className="text-[10px] text-neutral-600 hover:text-orange-400 font-mono underline decoration-neutral-800 underline-offset-4 transition-colors">Download MSI</Link>
                </div>
            </div>
          </OSCard>
          ...
          >
             <div className="flex flex-col gap-4 mt-auto">
                <div className="grid grid-cols-2 gap-2 h-10.5">
                   <div className="flex items-center justify-center text-[10px] font-mono text-neutral-500 bg-white/5 rounded-lg border border-white/5">Debian</div>
                   <div className="flex items-center justify-center text-[10px] font-mono text-neutral-500 bg-white/5 rounded-lg border border-white/5">RedHat</div>
                </div>
                <DownloadButton primary={activeOS === "linux"} label="Get AppImage" />
                <div className="text-center">
                    <Link href="#" className="text-[10px] text-neutral-600 hover:text-orange-400 font-mono underline decoration-neutral-800 underline-offset-4 transition-colors">Snap / Flatpak</Link>
                </div>
            </div>
          </OSCard>
          ...
          >
            <div className="flex flex-col gap-4 mt-auto">
                ...
                >
                    <Icon name="book" className="w-4 h-4 text-neutral-500 group-hover:text-orange-400 transition-colors" /> Setup Guide
                </Link>
                <p className="text-center text-[10px] text-neutral-600 font-mono">v2.4.0-stable</p>
            </div>
          </OSCard>
        ...
        <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
            <h2 className="text-sm font-bold text-neutral-500 uppercase tracking-widest mb-6 text-center">Or install via Terminal</h2>
            ...
            <div className="group bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden shadow-2xl relative transition-all hover:border-white/20">
                <div className="flex items-center border-b border-white/5 bg-black/40 px-4 pt-3 gap-2">
                    ...
                    <PMTab id="brew" label="Homebrew" icon="terminal" activePM={activePM} setActivePM={setActivePM} />
                    <PMTab id="winget" label="WinGet" icon="windows" activePM={activePM} setActivePM={setActivePM} />
                    <PMTab id="snap" label="Snap" icon="linux" activePM={activePM} setActivePM={setActivePM} />
                </div>
                ...
                <div className="p-8 bg-black/80 font-mono text-sm relative min-h-30 flex items-center backdrop-blur-sm">
                    ...
                    <div className="w-full">
                        {activePM === "brew" && <CopySnippet text="brew install --cask jiramo" terminalStyle />}
                        {activePM === "winget" && <CopySnippet text="winget install Jiramo.App" terminalStyle />}
                        {activePM === "snap" && <CopySnippet text="snap install jiramo" terminalStyle />}
                    </div>
                </div>
            </div>
        </div>
...
function OSCard({ id, title, isActive, icon, version, children, delay }: any) {
    return (
        ...
        >
            ...
            {isActive && <div className="absolute inset-0 bg-orange-500/5 rounded-3xl pointer-events-none"></div>}
            ...
            <div className="relative z-10 flex flex-col h-full">
                <div className="mb-6 flex justify-between items-start">
                    ...
                    {isActive && <div className="h-2 w-2 rounded-full bg-orange-500 shadow-[0_0_10px_currentColor] animate-pulse"></div>}
                </div>
                
                <div className="mb-8">
                    <h3 className={`text-xl font-bold mb-1 transition-colors ${isActive ? 'text-white' : 'text-neutral-200 group-hover:text-white'}`}>{title}</h3>
...
function DownloadButton({ primary, label }: { primary: boolean, label: string }) {
    return (
        <button className={`
        ...
        `}>
            <Icon name="download" className={`w-4 h-4 ${!primary && "text-neutral-500 group-hover:text-orange-400 transition-colors"}`} /> 
            {label}
        </button>
    )
}
...
function PMTab({ id, label, icon, activePM, setActivePM }: any) {
    ...
    return (
        <button
            ...
            `}
        >
            <Icon name={icon} className={`w-3.5 h-3.5 ${isActive ? 'text-orange-500' : 'text-neutral-600'}`} />
            {label}
            {isActive && <div className="absolute top-0 left-0 right-0 h-px bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.5)]"></div>}
        </button>
    )
}

function CopySnippet({ text, small = false, terminalStyle = false }: { text: string, small?: boolean, terminalStyle?: boolean }) {
    ...
    if (terminalStyle) {
        return (
            <div className="flex items-center justify-between w-full group cursor-pointer" onClick={handleCopy}>
                <div className="flex items-center gap-3">
                    ...
                    <code className="text-sm md:text-base text-neutral-200 group-hover:text-white transition-colors">
                        <span className="text-orange-500 font-bold">{text.split(' ')[0]}</span> {text.split(' ').slice(1).join(' ')}
                    </code>
                </div>
        ...
        >
            ...
            <div className="shrink-0">
                {copied ? (
                    ...
                    <Icon name="copy" className="w-3.5 h-3.5 opacity-50 group-hover:opacity-100 transition-opacity" />
                )}
            </div>
        </button>
    )
