    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <span key={i} className="text-orange-400 font-bold bg-orange-400/10 rounded px-0.5">
            {part}
          </span>
        ) : (
    ...
    <div className="relative flex flex-col justify-end">
      <div
        className={`
          ...
          glass border border-white/10 rounded-2xl p-2 flex flex-col gap-1
          transition-all duration-300 origin-bottom
          ${isOpen && localQuery ? "opacity-100 scale-100 visible z-50" : "opacity-0 scale-95 invisible -z-10"}
        `}
      >
        {isSearching ? (
          <div className="p-3 text-sm text-neutral-400 text-center animate-pulse">
            Ricerca in corso...
          </div>
        ) : results.length > 0 ? (
          results.map((post, index) => {
            ...
            return (
              <Link
                ...
                `}
              >
                <span className="text-sm font-medium text-white transition-colors line-clamp-1">
